...
> **策略（07-17 老師方向＋Steven 修訂版）**：台上**完全唔現場操作平台**——問題用**真實研究數據 dashboard** 講；demo 用**賽前預錄嘅「專心 vs 分心」對比影片**；Results 用截圖。PPT 行**多文字路線**（評判自己讀得明，唔使靠腦補），deck 已喺 Canva 完成（見 6.3）。真平台留返俾 Q&A／攤位（Part 5）。
