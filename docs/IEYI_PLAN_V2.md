...
**畫面設計方案（2026-07-15 Steven 提出並拍板）**：
- **閱讀階段**：材料放**螢幕中間**一張玻璃質感閱讀卡（闊約 60–70%、可上下捲動，字體大細跟平台現有排版）；**側邊 HUD 保留但精簡**——留低：專注 %、訊號質素 chip、本節倒數（3:00 起跳）、節數進度（例：第 2/5 節）、⚙ 設定掣；收起：速度／距離／航海圖／能量環（全部係航行專用，閱讀時冇意義）。背景照用而家嘅天空＋海，**無船無航道**；**天氣共感照行**（分心→起霧變暗，一望個海就知自己狀態）；**呼吸介入照觸發**（介入期間本節 3 分鐘倒數凍結，跟現有 pause 管線，唔會蝕咗閱讀時間）。
...
| D3-2 | 閱讀器：**螢幕右邊圓角面板** + sub-topic 分頁 + 3min/頁倒數（內嵌計時器）+ 提早揭頁 + 海景無船背景（收起船/航道/浮標）+ 最後頁「進行測驗」掣 | ✅ 07-16 |
...
> **註（D3-2 設計微調）**：原稿寫「閱讀卡放螢幕中間」，Steven 07-16 改為**放右邊**（左邊 focus HUD 照留，右邊閱讀，兩者唔打架）——已按新設計實裝。計時器由「側邊」改為**放入閱讀器裏面**（跟 Steven 指示）。

**依賴/待辦**：① 老師確認卷設計（同材料同卷 **＋ 兩組係咪都喺平台答卷**——見上面公平性提醒）；② Steven 提供材料一/材料二（**D3-2 已留白霸位，一有材料就填入 `pages/game/studyMaterials.js` 即得，唔使改其他 code**）；③ 實驗日期定咗話聲（要留返 buffer 出 CSV 教學）。
