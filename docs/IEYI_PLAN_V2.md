...
>
> **2026-07-20 第十四輪（Steven：官方比賽須知 PDF 對照＋截圖出貨）**：
> 54 **官方 delegation PDF 對照**（Steven 上載老師 IEYI 2026 須知＋隊員 deck）——發現手冊 Part 11／plan §6 舊時間表過時，已全部對正官方版：
>   - 🚨 **07-20 = poster 硬死線（今日）**：final A0 PDF 要 email 去 `wzy@cyt.edu.hk`；如要隊旗綵帶／彩印，07-20 前通知 WANG Ziyuan 老師（校方彩印上限 30 張 A4）；攤位裝飾每隊 **≤HK$100 並要留單**。
>   - **比賽在北京、五日制**（07-27 抵京 → 07-28 開幕＋評審下晝 → 07-29 評審上晝＋隊際交流＋撤展 → 07-30 頒獎 → 07-31 離京）——手冊 Part 11 由「本地兩日」改寫成北京五日程；plan §6 時間表補回「離港前要搞掂＋裝箱」。
>   - **3 分鐘英文硬限**（評判每隊最多停留 2 分鐘）＋官方要求備 **90–120 秒 core version**——手冊 Part 6 講稿由 3:50 壓到 **≤3:00**、另加 core version；Part 5 四人分工對正官方三角色（主講／示範＋技術／攤位長）。
...
>
> **2026-07-25 第十六輪（真頭帶實測 → EEG 全面收尾）**：
> 61 **真 EEG 首次跑通**，同場捉到四個真 bug：① HUD 訊號 chip **極性讀反**（bridge 送 100=完美，HUD 當 0=完美，所以頭帶放枱面都寫「訊號良好」）；② 頭帶滑咗仍然送封包，舊 code 只檢查「有冇封包」，**計時照行兼將整段記成分心**；③ 門檻跟模擬曲線調（穩定 50），但 NeuroSky eSense **40–60 係中性**，令正常清醒都判分心、呼吸提示狂彈——改用 eSense 分級（穩定 40／低 30／恢復 45）＋輕度平滑；④ eSense **0 係「算唔到」哨兵值唔係零分**，同樣要暫停計時。
> 62 **一個自製 regression 已修**：收緊訊號門檻時連 `waitForLiveEEG()`（判斷「連唔連得到」）都一齊收緊咗，令感測器未貼穩就報「連唔到」。已分開「條線通唔通」同「數據靚唔靚」兩個判斷。
> 63 **Bridge 崩潰修復**：`queue_sender` 一邊 `await ws.send()` 一邊迭代 client 集合，瀏覽器喺嗰刻離開即掟 `RuntimeError: Set changed size during iteration`，發送任務直接死 → **玩完一場 bridge 就唔再送數據**。改用快照迭代＋逐幀容錯＋client 離線唔再印 traceback。
> 64 **攤位循環體驗**：返主頁保持 socket 待機（唔使重開 bridge）、記住 EEG 站唔再重複彈確認、開場讀秒等到訊號可用先自動開波（30 秒上限）。
> 65 **HUD**：attention／meditation 加返可讀標籤＋即時數字（eSense 0 顯示 `--`），刪走右下角綠色 debug 框；狀態文字加行數上限、短螢幕加內部捲動——修正手機打橫被裁走內容。
> 66 **Q&A 重寫（手冊 Part 7）**：改成 **L1 人人問／L2 中度追問／L3 技術深入** 三層，每題標註對應官方評分格（🎨創意 40／💰市場 30／🛠️實用 30），另加「尷尬情況應對」表。理由：多數評判唔係技術專家，需要由淺答起。模組版本 `2026-07-25-5`。
>
...
## 6. 時間表（2026-07-20 對正官方 delegation PDF；比賽在**北京、五日制 07-27 → 07-31**，評審 **07-28 下晝 → 07-29 上晝**；code freeze **07-25** 不變）

> **關鍵改動（07-20）**：官方 PDF 確認比賽喺**北京**、五日制，唔係本地兩日。即係所有嘢（poster PDF 交咗、裝置叉滿、離線備份、綵排）都要**離港前搞掂**（估計 07-26／27 出發）。三條硬死線：**07-20 交 poster PDF**、**07-25 code freeze**、**離港日裝箱**。攤位有 **data card 上網**（每隊一張），但仍要離線備份。
>
> **🚨 07-20（今日）死線**：final A0 poster PDF email 去 `wzy@cyt.edu.hk`；要隊旗／彩印就今日前通知 WANG Ziyuan 老師（校方彩印上限 30 張 A4）。
>
> **剩低關鍵路徑幾乎全係人手＋硬件**：🔴 借 Windows 機＋EEG 裝置（最大 blocker）、跑 pilot 實驗、E1 真頭帶收尾、poster 定稿送交。

| 時段 | 做乜 |
|------|------|
| ✅ 已完成（07-06 → 07-16） | D2、D1、P1（連設定面板）、P2 Claude scope、U1 主體、E1 邏輯層、F1 報告、入場 hang 修復、模擬即秒載入、**D3 學習模式全套（含隊員教材）**——全部喺 main |
| **🚨 07-20（今日・poster 死線）** | **final A0 poster PDF email 去 `wzy@cyt.edu.hk`**（校方統一 A0 打印，唔使自己印）；要隊旗／彩印就今日通知 WANG Ziyuan 老師；裝飾每隊 ≤HK$100 留單。**Claude 已出齊 PPT／poster 截圖**（Results 正面趨勢／閱讀器／測驗船／晴天起霧），poster 差圖已補 |
| **07-20 → 07-24** | 🔴 **借 Windows 機＋EEG 裝置**（已過原定限期）；同老師確認「同材料同卷＋兩組都喺平台答卷」；招 2–3 個學生、定實驗日；🧪 **跑 pilot 實驗**（紙本 vs 平台，一鍵出 CSV）；**T2 實機 rehearsal ×5**（記斷線筆記）→ 交 Claude 做 **E1 收尾**；Vercel/DeepSeek 四步驗證；Supabase RLS 用 F1 條 SQL 核實 |
| **🎤 答辯準備（零現場操作版）** | **9 頁 PPT ✅ 已喺 Canva 完成**（S7 貼返 Results 截圖）；**拍 S6 對比影片**（專心 vs 分心，照手冊 **6.5**，~40 秒）；講稿 A–E（手冊 **6.4**）計時行到 **≤3:00**（官方硬限）＋練 **90–120 秒 core version**（評判可能只停 2 分鐘）；全程**英文**講；手提機開定真平台 standby 畀 Q&A |
| **07-25 → 離港前** | **Code freeze（07-25）**——之後唔加新功能。完整 rehearsal ×2、跨裝置 QA、故障演練（EEG 斷→Simulation、data card 慢→離線備份）、DEMO_MODE 開關拍板、電量策略（攤位冇電源！見手冊 Part 11）；**離線備份**（Results 截圖／對比影片／PPT／流程圖）入 USB＋雲 |
| **離港前一晚** | 裝置全部叉滿、AAA 電池換新、poster（校方印）＋物資裝箱（清單：手冊 Part 11）；確認 offline backup 播得到 |
| **07-27 抵京** | tentative 酒店 check-in 15:00–18:00 |
| **07-28（北京）** | 門開 08:30、開幕 09:00–10:00、**12:00 前攤位就緒**、**評審 14:00–17:00**；至少一人留守＋輪流去維修區（220V）充電 |
| **07-29（北京）** | **評審 09:00–12:00**、隊際交流 14:00–16:00、**撤展 16:00–17:00**（自行帶走所有嘢） |
| **07-30 → 07-31** | 頒獎＋閉幕 07-30 09:00–11:00；07-31 離京。（另：北京期間有**文化參訪＋IG Reel 作業**，主題「科技如何塑造當代人的生活」，見手冊 Part 11） |

**時間唔夠就保呢啲（順序）**：① **07-20 poster PDF 交出**（今日硬死線，缺席會冇 poster）；② T2 rehearsal（現場風險最大）＋ E1 收尾（要 T2 筆記）；③ D3 pilot 實驗（實在跑唔切就 demo 到、實驗改賽後補做，同老師預早講）。隱私政策頁同 P2 加碼位可以犧牲。
