...
- **學習模式 Study Mode（🚧 2026-07-15 立項，建置中；正式站會見到張卡但鎖住「Coming Soon」，做完先解鎖）**：第三個選項，對應負責老師嘅**紙本 vs 平台對照實驗**——揀學科（首發生物，「基礎/進階」兩級深度；化學/物理/歷史 🔒 Coming Soon）→ **閱讀階段**（sub-topic 分頁閱讀器，每頁最多 3 分鐘可提早跳；背景係同一片天空海景但**無船無航道**；天氣共感/呼吸介入/三路訊號輸入照用）→ **答題階段**（AI 預先生成＋老師審核鎖死嘅固定卷）→ **Study Results**（溫習/答題兩階段數據斬開＋CSV 匯出，學生用編號 S01/S02/S03 唔收真名）。詳見 plan §5 D3。**比賽 demo 唔倚賴佢**：訓練/挑戰模式一行 code 都冇郁。
