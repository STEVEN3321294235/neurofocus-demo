...
> ㊲ **⏱️ Demo 時間重新分析（Steven 07-17 指正）**：學習模式每頁鎖 15 秒 ×5 頁＋10 條 MC，完整流程機械下限 ≈2.5–3 分鐘，**現場行唔晒**。新方案（手冊 6.2）：現場只 demo 閱讀第 1 頁＋分心→呼吸介入；測驗用截圖；儀表板**切去朝早預先完成場次嘅 Results tab**；另備成套錄屏做保險。
...
>
> **2026-07-18 第十輪（PPT prompt 詳細化＋語言分析・純文件）**：
> ㊴ **手冊 6.3 大改（Steven 指示）**，拆成五個小節：**6.3.1 語言拍板分析——建議 slides 全英文＋口頭按評判雙語**（IEYI 國際評判團、中英並排令每頁字量翻倍違反「3 秒睇明」、同 A0 海報／英文 UI 截圖一致；保險：如官方確認中文評審，照大綱表中文標題 15 分鐘換字，建議 07-20 前問清楚）；**6.3.2 文獻數據卡**——9 個可引用真數字連出處（Gloria Mark 47 秒／打斷後 25 分鐘、mind-wandering 47%、慢呼吸 Zaccaro 2018＋Balban 2023、retrieval practice g≈0.5–0.6、neurofeedback 證據混合等），附「每頁最多一個數字」同「唔好講過龍」警示（例：唔好用「8 秒金魚」都市傳說）；**6.3.3 Slide 7 儀表板三個「顯示差異」位**——兩階段差 4–8pp（vigilance decrement）、跨場恢復改善 20–40%（神經回饋漸進學習曲線）、介入全救返（12 秒一輪＋慢呼吸文獻），配**誠實鐵律**：mock 必標 "Illustrative mock-up"、唔可以講成 pilot 結果、真 CSV 一出即棄示意；**6.3.4 大綱速覽表**（中文標題留做對照＋中文版後備）；**6.3.5 Canva prompt 詳細版**——由 20 行擴到逐頁 spec（全局色板／字體／每頁鐵律＋逐頁標題文字圖位數據 callout＋第 9 頁隱藏備用錄屏頁＋隊員自查 5 條）。Part 10 checklist 對應加 3 項。**冇 code 改動，唔使 bump 版本。**
...
| **🎤 答辯準備（07-17 起・老師新方向）** | 隊員用 **Canva 砌 8 頁 PPT**（照手冊 **6.3** 個 prompt 直接貼入去）＋cap 三張截圖（閱讀/測驗/Results）＋Slide 2 條片離線版；講稿 A–E（手冊 **6.4**）計時行一次 ≤3.5 分鐘；錄一條完整學習流程**備用錄屏**；比賽朝早用 demo 帳戶**預先完成一場** study session、開定 Results tab（細船已拍板唔做，冇 code 改動） |
