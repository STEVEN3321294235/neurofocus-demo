...
>
> **2026-07-16 第六輪（D3 學習模式開始實裝：D3-1＋D3-2）**：
> ⑰ **D3-1 完成**：Setup 加咗第三張目標卡「學習模式 / Study Mode」，**正式站顯示但鎖住**（「Coming Soon」撳唔入，跟 Steven 07-16 拍板）；開發／測試用 `localStorage.setItem('nf_study_dev_unlock','1')` 解鎖。揀咗之後行學科（生物可入＋化學/物理/歷史 🔒）→ 深度（基礎/進階）→ 再共用返輸入來源＋相機步驟。
> ⑱ **D3-2 完成**：閱讀器係螢幕**右邊**一塊圓角玻璃面板（喺同一片天空＋海景度，但**收起隻船、航道虛線、浮標**——係 visible 開關唔係拆場景，所以之後玩訓練/挑戰會攞返晒）。每頁 **3 分鐘**上限（計 accumulatedPlayTime，所以開設定/呼吸介入/直向警告/相機斷全部經現有 pause 管線一齊凍結頁計時，唔會蝕閱讀時間）；「下一頁」揭頁，最後一頁個掣寫「進行測驗」，撳咗顯示佔位提示（等 D3-4 接真卷）。**課文內容住先留白**（等老師材料）。計時器沿用訓練倒數嗰套 look，但放咗入閱讀器裏面。
> ⑲ 已 headless 測試：學習模式 setup→閱讀 boot、計時倒數、提早揭頁、最後一頁測驗掣、設定暫停凍結頁計時全部 ✅；挑戰模式迴歸（船照行、題照出、設定暫停）✅；鎖住卡撳唔入 ✅。模組版本 bump 到 `2026-07-16-1`。
...
| D3 學習模式立項（老師對照實驗 → 第三 Session 目標） | commit `741de330`（2026-07-15） |
| **D3-1 Setup 第三卡（顯示但鎖住）＋學科／深度步驟** | commit `8ab95b45`（2026-07-16） |
| **D3-2 右邊圓角閱讀器（3min/頁、內嵌計時、無船海景、揭頁／測驗掣）** | commit `8ab95b45`（2026-07-16） |
...
- 上線策略：**建置期第三張卡即刻上正式站但鎖住**——顯示「學習模式 · Coming Soon」撳唔入（攤位/評判見到路線圖），D3 全部做完驗收先解鎖入口。
...
**實作拆步**（每步一 commit、逐步驗收；建置期第三張卡**顯示但鎖住**〔Steven 拍板 07-15〕，D3-6 驗收完先解鎖）：
| 步 | 內容 | 狀態 |
|---|---|---|
| D3-1 | Setup 第三張卡「學習模式 · Coming Soon」（顯示但鎖住）+ 學科選擇 UI（生物可入＋化學/物理/歷史 3 鎖）+ 深度（基礎/進階）+ state/route 底盤 | ✅ 07-16 |
| D3-2 | 閱讀器：**螢幕右邊圓角磨砂面板（闊約 60vw、隨比例縮、唔重疊左邊 HUD）**、跟深淺色變白/黑磨砂；**筆記式排版**（標題/段落/重點列表/名詞解釋/重點框）；內嵌計時器；**每頁最少讀 1 分鐘先解鎖「下一頁」、最多 3 分鐘自動揭頁**；海景無船背景（收起船/航道/浮標）；最後頁「進行測驗」掣 | ✅ 07-16（含 Steven 07-16 第二輪微調） |
| D3-3 | 閱讀期間專注監測 + 介入重用（介入已凍結頁倒數）+ 溫習階段數據記錄（每頁用時已收集，待接落 Results） | ⏳ 下一步 |
| D3-4 | 答題階段：船隻＋挑戰式題目 HUD 返場；固定審核題庫（repo JSON per 材料）+ 每題用時記錄 +（demo 用）AI 即場出題開關 | ⏳ |
| D3-5 | Study Results 兩階段版面 + CSV 匯出 | ⏳ |
| D3-6 | 雙語文案 + 解鎖入口（除低 Coming Soon 鎖）+ docs 更新 | ⏳ |

> **註（D3-2 兩輪設計微調）**：① 07-15 原稿寫「閱讀卡放螢幕中間」，07-16 Steven 改為**放右邊**（左邊 focus HUD 照留）——已實裝；計時器由「側邊」改為**放入閱讀器裏面**。② 07-16 第二輪 Steven 再定：闊度**拉闊到 60vw**（隨比例縮、唔重疊 HUD）、背景改**磨砂＋跟深淺色變白/黑**、**每頁最少讀 1 分鐘**先可揭頁、內文用**筆記式排版**——全部已實裝並 headless 驗過。

**預製教材（07-16 Steven 要求，未收到老師材料前先由 Claude 預製）**：生物科「細胞」單元，兩級深度已寫入 `pages/game/studyMaterials.js`：
- **基礎（初中程度）**：《細胞：生命的基本單位》——細胞學說、動植物細胞、胞器功能、擴散入門（5 節筆記）＋ 5 條 preset MC。
- **進階（高中程度）**：《細胞膜與物質運輸》——流體鑲嵌模型、被動/主動運輸、滲透與水勢、表面積體積比（5 節筆記）＋ 5 條 preset MC。
- 全部**雙語（中/英）**、筆記式結構、MC 跟遊戲現有題目 schema（4 選項＋正解＋解釋），可直接畀老師審核／取代。**呢啲係 Claude 預製佔位版**：老師一有正式材料就換入同一個檔案即得。

**依賴/待辦**：① 老師確認卷設計（同材料同卷 **＋ 兩組係咪都喺平台答卷**——見上面公平性提醒）＋審核／取代 Claude 預製嘅生物教材同 MC；② 若老師想用其他科目/課題，提供材料換入 `studyMaterials.js`；③ 實驗日期定咗話聲（要留返 buffer 出 CSV 教學）。
